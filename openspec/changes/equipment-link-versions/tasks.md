...
- [ ] 2.5 Verify that a loadout the current table cannot represent publishes no fragment and no
      ...
      `refused` carrying the mount, the address fragment is unchanged, and the loadout on the bench
      is untouched.
...
- [ ] 3.2 Give the generator the minting path the build-link generator carries: where the file for
      ...
      `generate-build-link-codec-tables.mjs` does. Today raising `TABLE_VERSION` to 2 fails with
      "table 2 is missing", so verify in `test:scripts`, against a temporary directory, that raising
...
- [ ] 3.4 Add the equipment tables to `pnpm run codec:capacity` so each published table's encoded
      size is reported beside the build-link tables, and verify the command reports a figure for
      every committed equipment version.

## 4. Prove an old link stays readable

- [ ] 4.1 Capture the corpus from the shipped application before this change lands: encode loadouts
      with the current code, copy the fragments out, and commit them in
      `published-equipment-links.fixture.json` as literals with the loadout each opens to and the
      modifications its mounts carry, keyed by table version. The fixture is never regenerated by the
      changed code. Verify every entry's payload version field matches the key it is filed under, and
      that each fragment decodes to its recorded loadout under the pre-change code.
...
- [ ] 5.1 State in `docs/equipment-link-codec.md` that a published table is immutable, that a payload
      names the table that decodes it, and that the registry is synchronous because the equipment
      table is 3,021 bytes where a build-link table is about 198 KB. Verify every size and corpus
      figure in the document is the measured one, and that the per-table size and the table count are
      both stated.
