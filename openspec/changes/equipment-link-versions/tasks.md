...
- [x] 1.0 Capture the corpus before anything below changes the codec. Encode loadouts with the code
...
- [x] 1.1 Export an `EquipmentLinkCodecTables` type derived from `equipment-link-table-1.json`, and
      verify `pnpm run typecheck` passes with the committed table assigned to it.
- [x] 1.2 Move the encode and decode pair into a `createEquipmentLinkCodec(tableVersion, table)`
      factory, with the seven module constants and every `table.` read inside it. The module keeps
      one table import until task 2.1, used only to build the codec its exported pair calls. Verify
      the factory holds every table read, and that the existing `equipment-link-codec.spec.ts` passes
      unchanged against a codec built on table 1.
- [x] 1.3 Have the factory's encoder write the table version it was built with rather than a module
...
- [x] 2.1 Add a registry beside the codec: a `ReadonlyMap` holding one codec per version, each built
      ...
      the suites still do.
- [x] 2.2 Have `decodeEquipmentLinkFragment(fragment, codecs = EQUIPMENT_CODECS_BY_TABLE_VERSION)`
...
- [x] 2.3 Refuse a payload naming a version the given map does not hold with
...
- [x] 2.4 Keep `encodeEquipmentLinkFragment` writing the current version whatever version was read,
...
- [x] 2.5 Drive `LoadoutLinkCoordinator` with a codec that refuses the open loadout. Verify the link
...
- [x] 3.1 Derive the generator's output path and its published-version set from `TABLE_VERSION`
...
- [x] 3.2 Give the generator a minting path. Where the file for `TABLE_VERSION` is absent, it writes
      ...
      regression guard.
- [x] 3.3 Add the guard that holds every version below `TABLE_VERSION` to the hash it declares, as
...
- [x] 4.1 Commit what task 1.0 captured as `published-equipment-links.fixture.json`: the fragments
...
- [x] 4.2 Add a suite that opens every fixture fragment through `decodeEquipmentLinkFragment` and
      asserts the recorded loadout, and verify it fails when an entry is filed under the wrong
      version.
- [x] 4.3 Assert in that suite that every version the shipped registry holds has at least one corpus
...
- [x] 4.4 Assert that every fixture fragment re-encodes to the current version and reopens on the
      same loadout, and verify the held weapons and modifications survive the rewrite.
- [x] 4.5 Exercise version selection without committing a table. Build a test-only version 2 table,
...
- [x] 5.1 Rewrite the passages of `docs/equipment-link-codec.md` this change makes false:
...
- [x] 5.2 State in the same document that a published table is immutable. State that a payload names
...
- [x] 5.3 Extend the `equipment/link` journey in `e2e/equipment-link.spec.ts` with two literal `e.`
...
- [x] 5.4 Add two assertions to the `equipment/link` entry in `e2e/coverage-ledger.ts`. The first:
      ...
      refused where the Commander is. Word the second so it does not read as a duplicate of the
      existing line about a link this version cannot read. Verify `pnpm run policy:specs` reports no
      violation, and that the journey carries a test for each assertion added.

## 6. Verification

- [x] 6.1 Run `pnpm run typecheck`, `pnpm run format:check` and `pnpm run policy`, and verify each
      passes.
