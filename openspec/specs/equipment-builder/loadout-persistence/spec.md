...
reason. The refusal MUST be said of the loadout that could not be written, not of
a link that could not be read: nothing arrived, and a Commander told their own
loadout could not be read would go looking for a link that never existed.
...
Every version the application carries MUST have at least one link in a corpus held as literal
fragments, each recorded with the loadout it opens to and never regenerated by the code under test,
so a table that stops decoding the links it wrote fails the build. Table 1 is the only published
version, so the corpus pins one version today and "A link written by an older published version" is
evidenced against a shipped table by the change that mints table 2.
